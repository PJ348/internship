  ...
  `;
      ...
      `;
        ...
        btnNext?.addEventListener('click', () => {
            if (isAnimating || sliderContainer.children.length === 0)
                return;
            isAnimating = true;
            const firstChild = sliderContainer.children[0];
        ...
        btnPrev?.addEventListener('click', () => {
            if (isAnimating || sliderContainer.children.length === 0)
                return;
            isAnimating = true;
            const lastChild = sliderContainer.children[sliderContainer.children.length - 1];
            const firstChild = sliderContainer.children[0];
            const colWidth = firstChild.offsetWidth;
    ...
    const allCompaniesGrid = document.getElementById('all-companies-grid');
    if (allCompaniesGrid) {
        let gridHtml = '';
        // ดึงข้อมูลทั้งหมดที่มีในระบบจริงๆ (ไม่มีการวนลูป)
    ...
    // ระบบเลื่อนเส้นใต้ Navbar และดักจับการเข้า Profile
    ...
    if (navMenu && navItems.length > 0 && navIndicator) {
        ...
        // เช็ก URL ตอนเปิดหน้าต่าง ว่าอยู่หน้า Profileมั้ย
        setTimeout(() => {
            const currentPath = window.location.pathname;
            if (currentPath.includes('profile.html') && isLoggedIn) {
                moveIndicator(navItems[1]); // ขีดใต้ Profile
            }
            else {
                moveIndicator(navItems[0]); // ขีดใต้ Feed
            }
        }, 100);
        // ดักจับเวลากดปุ่ม Profile (ตำแหน่งที่ 2 คือ index 1)
        const profileLink = navItems[1];
        if (profileLink) {
            profileLink.addEventListener('click', (e) => {
                if (!isLoggedIn) {
                    e.preventDefault(); // บล็อกไม่ให้ไปหน้า profile.html
                    window.location.href = './login.html'; // บังคับเด้งไปหน้าเข้าสู่ระบบ
                }
            });
        }
    }
    ...
    const checkAuthState = () => {
        ...
        if (isLoggedIn) {
            ...
            // ถ้าล็อกอินแล้ว ให้ซ่อนกล่องลงทะเบียน
            ...
            // ถ้ายังไม่ล็อกอิน ให้แสดงกล่องลงทะเบียน
